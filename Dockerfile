...
RUN if [ ${APP_WITH_SCHEDULER} = true ]; then \
  wget -q "https://github.com/aptible/supercronic/releases/download/v0.2.26/supercronic-linux-amd64" -O /usr/bin/supercronic && \
  chmod +x /usr/bin/supercronic && \
  mkdir -p /etc/supercronic && \
  echo "* * * * * php ${ROOT}/artisan schedule:run --verbose --no-interaction" > /etc/supercronic/laravel && \
  echo "0 0 * * 0 php ${ROOT}/artisan telescope:prune" >> /etc/supercronic/laravel; \
fi
...
COPY deploy/octane/supervisord* /etc/supervisor/conf.d/
COPY deploy/octane/php.ini /usr/local/etc/php/conf.d/octane.ini
COPY deploy/octane/.rr.prod.yaml ./.rr.yaml

# # SCRIBE GENERATE DOCS
# RUN if [ -f "artisan" ]; then \
#   ls -l artisan && \
#   php artisan --version && \
#   php artisan scribe:generate --force --ansi || true && \
#   ls -l /var/www/html/.scribe || true; \
# fi


RUN chmod +x deploy/octane/entrypoint.sh

# # # SCRIBE DOCS
RUN mkdir -p /var/www/html/.scribe && \
    chown -R octane:octane /var/www/html && \
    chmod -R ug+rwx /var/www/html/.scribe && \
...
RUN cat deploy/octane/utilities.sh >> ~/.bashrc

EXPOSE 9000

ENTRYPOINT ["deploy/octane/entrypoint.sh"]